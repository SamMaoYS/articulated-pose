...
def single_transformation_estimator(dataset, best_inliers=None):
    ...
    rotation, scale, translation = transform_pts(
        dataset["source"][sample_idx, :], dataset["target"][sample_idx, :]
    )
    strans = dict()
...
def single_transformation_verifier(dataset, model, inlier_th):
    ...
    res = (
        dataset["target"].T
        - model["scale"] * np.matmul(model["rotation"], dataset["source"].T)
        - model["translation"].reshape((3, 1))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--item", default="oven", help="object category for benchmarking"
    )
    parser.add_argument(
        "--domain", default="unseen", help="which sub test set to choose"
    )
    parser.add_argument(
        "--nocs", default="npcs", help="type of NOCS used to train the model"
    )
    ...
    rts_all = pickle.load(
        open(
            directory + "/{}_{}_{}_rt.pkl".format(args.domain, args.nocs, args.item),
            "rb",
        )
    )
    ...
    for i in range(len(test_group)):
        try:
            ...
            f = h5py.File(
                my_dir + "/results/test_pred/{}/{}.h5".format(args.num_exp, basename),
                "r",
            )
            ...
            for j in range(num_parts):
                ...
                best_model, best_inliers = ransac(
                    dataset,
                    single_transformation_estimator,
                ...
                target0_fit = best_model["scale"] * np.matmul(
                    best_model["rotation"], source0.T
                ) + best_model["translation"].reshape((3, 1))
    ...
    for j in range(num_parts):
        print(
            ...
            "baseline: {}".format(np.array(r_raw_err["baseline"][j]).mean()),
        )
        print(
            "mean translation err of part {}: \n".format(j),
            "baseline: {}".format(np.array(t_raw_err["baseline"][j]).mean()),
        )
