...
from lib.d3_utils import (
    axis_diff_degree,
    dist_between_3d_lines,
    rot_diff_rad,
    rot_diff_degree,
)
...
if __name__ == "__main__":
    ...
    for k in range(30):
        curr_file = "{}_{}_{}_{}_rt_ours_{}_{}.pkl".format(
            baseline_exp, args.domain, args.nocs, args.item, choose_threshold, k
        )
    ...
    for i in range(len(basenames["nonlinear"])):
        ...
        try:
            ...
            translation_list = []
            for j in range(num_parts):
                nocs_pred_final["pn"][part_idx_list_pred[j], :] = nocs_pred["pn"][
                    part_idx_list_pred[j], j * 3 : j * 3 + 3
                ]
                if nocs_pred["gn"].shape[1] == 3:
                    nocs_pred_final["gn"][part_idx_list_pred[j], :] = nocs_pred["gn"][
                        part_idx_list_pred[j], :3
                    ]
                else:
                    nocs_pred_final["gn"][part_idx_list_pred[j], :] = nocs_pred["gn"][
                        part_idx_list_pred[j], j * 3 : j * 3 + 3
                ...
                print(
                    "scale, translation from global NOCS to part NOCS is: ",
                    scale,
                    translation,
                )
            ...
            for j in range(num_parts-1):
                ...
                prediction_dict[basenames["nonlinear"][i]][j] = {}
                prediction_dict[basenames["nonlinear"][i]][j]["joint_pt"] = joint["p"].tolist()[0]
                prediction_dict[basenames["nonlinear"][i]][j]["joint_axis"] = joint["l"].tolist()[0]
                prediction_dict[basenames["nonlinear"][i]][j]["rotation"] = r[0].tolist()
                prediction_dict[basenames["nonlinear"][i]][j]["translation"] = t[0].tolist()
                prediction_dict[basenames["nonlinear"][i]][j]["scale"] = s[0].tolist()
            ...
            for j in range(num_parts-1):
                ...
                joint = {}
                joint["p"] = (
                    np.dot(s_g[0] * t_joint_pt.reshape(1, 3), rt_g[0][:3, :3].T)
                    + rt_g[0][:3, 3]
                )
                joint["l"] = np.dot(
                ...
                prediction_dict[basenames["nonlinear"][i]][j]["joint_pt_gt"] = joint["p"].tolist()[0]
                prediction_dict[basenames["nonlinear"][i]][j]["joint_axis_gt"] = joint["l"].tolist()[0]
