...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--domain", default="unseen", help="which sub test set to choose"
    )
    parser.add_argument("--nocs", default="ANCSH", help="which sub test set to choose")
    parser.add_argument(
        "--item", default="eyeglasses", help="object category for benchmarking"
    )
    ...
    if args.nocs == "ANCSH":
        baseline_file = directory + "/{}_{}_{}_rt_pn.pkl".format(
            args.domain, "ANCSH", args.item
        )
    else:
        baseline_file = directory + "/{}_{}_{}_{}_rt_gn.pkl".format(
            main_exp, args.domain, "NAOCS", args.item
        )

    pn_gt_file = my_dir + "/results/pickle/{}/{}_{}_{}_rt.pkl".format(
        main_exp, args.domain, "ANCSH", args.item
    )
    gn_gt_file = my_dir + "/results/pickle/{}/{}_{}_{}_rt.pkl".format(
        main_exp, args.domain, "NAOCS", args.item
    )
    ...
    valid_files = []
    for k in range(30):
        curr_file = "{}_{}_{}_{}_rt_ours_{}_{}.pkl".format(
            npcs_exp, args.domain, "ANCSH", args.item, choose_threshold, k
        )
    ...
    print(
        "For {} object, {} nocs, mean rotation err per part is: ".format(
            args.domain, args.nocs
        )
    )
    ...
    print(
        "For {} object, {} nocs, mean translation err per part is: ".format(
            args.domain, args.nocs
        )
    )
    for item in test_items:
    ...
    print(
        "For {} object, {} nocs, 5 degrees accuracy per part is: ".format(
            args.domain, args.nocs
        )
    )
    for item in test_items:
    ...
    print(
        "For {} object, {} nocs, 5 degrees, 5 cms accuracy per part is: ".format(
            args.domain, args.nocs
        )
    )
    for item in test_items:
    ...
    for instance in test_ins:
        if args.item in ["drawer"]:
            ...
            path_urdf = (
                group_dir + "/sapien/objects/" + "/" + args.item + "/" + instance
            )
        ...
        for p in range(num_parts):
            ...
            nocs_corner[0] = (
                ...
                - 0.5 * (norm_corner[1] - norm_corner[0]) * norm_factor
            )
            nocs_corner[1] = (
                np.array([0.5, 0.5, 0.5]).reshape(1, 3)
                + 0.5 * (norm_corner[1] - norm_corner[0]) * norm_factor
            ...
            if args.item in ["drawer"]:
                nocs_corner[0] = (
                    np.dot(nocs_corner[0].reshape(1, 3) - 0.5, rot_mat.T) + 0.5
                )
                nocs_corner[1] = (
                    np.dot(nocs_corner[1].reshape(1, 3) - 0.5, rot_mat.T) + 0.5
                )
    ...
    for i in range(len(basenames["nonlinear"])):
        ...
        try:
            ...
            for key in ["baseline", "nonlinear"]:
                ...
                cur_data = datas[key]
                if (
                    basename not in cur_data
                    or cur_data[basename]["scale"] is None
                    or cur_data[basename]["scale"] is []
                    or np.any(np.isnan(cur_data[basename]["translation"][key]))
                ):
                ...
                if key == "nonlinear":
                    f = h5py.File(
                        my_dir
                        + "/results/test_pred/{}/{}.h5".format(npcs_exp, basename),
                        "r",
                    )
                    ...
                    f = h5py.File(
                        my_dir
                        + "/results/test_pred/{}/{}.h5".format(main_exp, basename),
                        "r",
                    )
                ...
                for j in range(num_parts):
                    ...
                    if nocs_pred.shape[1] == 3:
                        ...
                        centered_nocs = (
                            nocs_pred[partidx["pred"][j], 3 * j : 3 * (j + 1)] - 0.5
                        )
                    scale_pred = 2 * np.max(abs(centered_nocs), axis=0)
                    bboxs3d["pred"].append(
                        get_3d_bbox(
                            scale_pred, shift=np.array([1 / 2, 1 / 2, 1 / 2])
                        ).transpose()
                    )
                    boundary["canon"].append(-scale_pred[0] / 2 + 0.5)

                    shifted_part_canon = np.dot(
                        np.concatenate(
                            [
                                input_pts[partidx["pred"][j], :3],
                                np.ones((len(partidx["pred"][j]), 1)),
                            ],
                            axis=1,
                        ),
                        np.linalg.pinv(rt_0.T),
                    )[:, :3]
                    boundary["dynam"].append(np.min(shifted_part_canon[:, 0]))
                    partidx["gt"].append(np.where(cls_per_pt_gt == j)[0])
                    scale_gt = (
                        bbox3d_all[instance][j][1][0] - bbox3d_all[instance][j][0][0]
                    )
                    bboxs3d["gt"].append(
                        get_3d_bbox(
                            scale_gt, shift=np.array([1 / 2, 1 / 2, 1 / 2])
                        ).transpose()
                    )
                    scale_err_per_part.append(
                        np.linalg.norm(scale_pred * s[j] - scale_gt * s_gt[j])
                    )
                    volume_err_per_part.append(
                        scale_pred[0]
                        * scale_pred[1]
                        * scale_pred[2]
                        * s[j]
                        / (scale_gt[0] * scale_gt[1] * scale_gt[2] * s_gt[j][0])
                        - 1
                    )
    ...
    for key, cur_data in datas.items():
        ...
        for i in range(len(basenames["nonlinear"])):
            try:
                ...
                basename = basenames["nonlinear"][i]
                if (
                    cur_data[basename]["scale"] is None
                    or cur_data[basename]["scale"] is []
                    or np.any(np.isnan(cur_data[basename]["translation"][key]))
                    or basename not in datas["pn_gt"]
                ):
                    # print('wrong data for scale or nan: ', basename)
                    continue
                ...
                if (
                    datas["pn_gt"][basename]["rt"] is None
                    or datas["pn_gt"][basename]["scale"] is None
                ):
                    continue
                ...
                for j in range(1, num_parts):
                    ...
                    if args.nocs == "NAOCS" and key == "nonlinear":
                        ...
                        t_diff_pred = (
                            boundary_all[key][basename]["dynam"][j]
                            - boundary_all[key][basename]["canon"][j]
                        )
                        t_diff_pred = np.dot(
                            r0, np.array([t_diff_pred, 0, 0]).reshape(3, 1)
                        ).reshape(-1)
                    r0 = rt_p[0][:3, :3]
                    r1 = rt_p[j][:3, :3]
    ...
    if args.item == "drawer":
        print(
            "For {} object, {} nocs, mean relative translation err per part is: ".format(
                args.domain, args.nocs
            )
        )
        for item in test_items:
        ...
        print(
            "For {} object, {} nocs, mean relative rotation err per part is: ".format(
                args.domain, args.nocs
            )
        )
        for item in test_items:
