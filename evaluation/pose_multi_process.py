...
if __name__ == "__main__":
    ...
    problem_ins = []
    print(
        "we have {} testing data for {} {}".format(
            len(test_group), args.domain, args.item
        )
    )
    ...
    rts_all = pickle.load(
        open(
            my_dir
            + "/results/pickle/{}/{}_{}_{}_rt.pkl".format(
                main_exp, args.domain, args.nocs, args.item
            ),
            "rb",
        )
    )
    ...
    file_name = directory + "/{}_{}_{}_{}_rt_ours_{}.pkl".format(
        baseline_exp, args.domain, args.nocs, args.item, choose_threshold
    )
    ...
    for process in processes:
        process.join()
    print(
        "Process {} took {} seconds, with average {} seconds per data".format(
            num_per_cpu * cpuCount,
            time.time() - starttime,
            (time.time() - starttime) / (num_per_cpu * cpuCount),
        )
    )
