...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_file",
        default="./cfg/network_config.yml",
        help="YAML configuration file",
    )
    parser.add_argument(
        "--name_data", default="shape2motion", help="name of the dataset we use"
    )
    parser.add_argument(
        "--item", default="eyeglasses", help="name of the dataset we use"
    )
    parser.add_argument(
        "--num_expr", default=0.01, help="small set data used for testing"
    )
    parser.add_argument(
        "--nocs_type", default="ancsh", help="whether use global or part level NOCS"
    )  # default A/B/C
    parser.add_argument(
    ...
    parser.add_argument(
        "--pred_joint",
        action="store_true",
        help="whether we want to predict joint offsets",
    )
    parser.add_argument(
        "--pred_joint_ind",
        action="store_true",
        help="whether we want to predict joint offsets index",
    )
    parser.add_argument(
        "--early_split",
        action="store_true",
        help="whether we want to early split for joints prediction",
    )
    parser.add_argument(
        "--early_split_nocs",
        action="store_true",
        help="whether we want to split for two nocs heads",
    )
    ...
    if is_debug:
        output_msg = "max_parts:  {}\n root_data: {}\n nocs_type : {}\n".format(
            n_max_parts, root_data, nocs_type
        )
        print(output_msg)
    ...
    net = Network(
        n_max_parts=n_max_parts, config=conf, is_new_training=not should_restore
    )
