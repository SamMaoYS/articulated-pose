...
def build_pointnet2_cls(scope, point_cloud, out_dims, is_training, bn_decay):
    with tf.variable_scope(scope):
        ...
        results = []
        for idx, out_dim in enumerate(out_dims):
            current_result = tf_util.fully_connected(
                net, out_dim, activation_fn=None, scope="fc3_{}".format(idx)
            )
            results.append(current_result)
...
def build_pointnet_cls(
    scope, point_cloud, features, out_dims, is_training, bn_decay, reuse=False
):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        net = net_shared
        net_shared = tf_util.fully_connected(
            net_shared,
            128,
            bn=True,
            is_training=is_training,
        ...
        net_shared = tf_util.dropout(
            net, keep_prob=0.4, is_training=is_training, scope="dp1"
        )
        ...
        for idx, fc2_dim in enumerate(fc2_dims):
            ...
            net = tf_util.dropout(
                net, keep_prob=0.4, is_training=is_training, scope="dp2_{}".format(idx)
            )

        results = []
        # use net_shared branch
        current_result = tf_util.fully_connected(
            net_shared, out_dims[0], activation_fn=None, scope="fc3_{}".format(0)
        )
        results.append(current_result)
        ...
        for idx, out_dim in enumerate(out_dims[1:]):
            current_result = tf_util.fully_connected(
                net, out_dim, activation_fn=None, scope="fc3_{}".format(idx + 1)
            )
            results.append(current_result)
...
def build_pointnet_cls_simple(
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        #
        ap1 = tf.layers.average_pooling1d(
            conv4, pool_size=num_point, strides=num_point
        )  # learn from global feature
        net = tf.reshape(ap1, [-1, 1024])
        # print('After average pooling: ', ap1)
        net = tf_util.fully_connected(
            net, 512, bn=True, is_training=is_training, scope="fc1"
        )
        net = tf_util.dropout(net, keep_prob=0.5, is_training=is_training, scope="dp1")
        net = tf_util.fully_connected(
            net, 256, bn=True, is_training=is_training, scope="fc2"
        )
        net = tf_util.dropout(net, keep_prob=0.5, is_training=is_training, scope="dp2")
