...
if __name__ == "__main__":
    ...
    base_path = (
        my_dir + "/dataset/"
    )  # with a lot of different objects, and then 0001/0002/0003/0004/0005
    # >>>>>>>>>>>>>>>>>>>>>>>> config end >>>>>>>>>>>>>>>>>>>>>>>>>>>>#
    ...
    for obj_name in all_objs:
        object_joints_per = {}
        instances_per_obj = sorted(
            glob.glob(base_path + dataset + "/objects/" + obj_name + "/*")
        )
        ...
        for sub_dir in selected_objs:  # regular expression
            ...
            save_dir = (
                ...
                + "/"
                + dataset
                + "/urdf/{}/{}".format(obj_name, instance_name)
            )  # todo
            ...
            with open(json_name[0]) as json_file:
                ...
                for i in range(num - 1):
                    ...
                    while joint_dict[child_name] != "dof_rootd":
                        print(
                            "joint {} now looking at child {}, has parent {}".format(
                                i, child_name, joint_dict[child_name]
                            )
                        )
                ...
                # >>>>>>>>>>> start parsing urdf,
                children = [Element("link", name=links_name[i]) for i in range(num)]
                joints = [
                ...
                node_inertial = XML(
                    """<inertial><origin rpy="0 0 0" xyz="0 0 0"/><mass value="1.0"/><inertia ixx="0.9" ixy="0.9" ixz="0.9" iyy="0.9" iyz="0" izz="0.9"/></inertial>"""
                )
                # >>>>>>>>>>>. 1. links
                for i in range(num):
                    ...
                    if dof_name == "dof_rootd":
                        origin = SubElement(
                            visual, "origin", rpy="0.0 0.0 0.0", xyz="0 0 0"
                        )
                    else:
                        origin = SubElement(
                            visual,
                            ...
                            rpy="0.0 0.0 0.0",
                            xyz="{} {} {}".format(
                                links_pos[i][0], links_pos[i][1], links_pos[i][2]
                            ),
                        )
                    ...
                    if i == 0:
                        mesh = SubElement(
                            geometry,
                            "mesh",
                            filename="{}/part_objs/none_motion.obj".format(sub_dir),
                        )
                    else:
                        mesh = SubElement(
                            geometry,
                            "mesh",
                            filename="{}/part_objs/{}.obj".format(sub_dir, dof_name),
                        )
                ...
                for i in range(num - 1):
                    ...
                    parent = SubElement(
                        joints[i], "parent", link=links_name[index_parent]
                    )
                    ...
                    origin = SubElement(
                        ...
                        "origin",
                        xyz="{} {} {}".format(
                            joints_pos[i][0], joints_pos[i][1], joints_pos[i][2]
                        ),
                        rpy="0 0 0",
                    )
                    ...
                    if joints_type[i] == "revolute":
                        axis = SubElement(
                            ...
                            "axis",
                            xyz="{} {} {}".format(
                                joints_axis[i][0], joints_axis[i][1], joints_axis[i][2]
                            ),
                        )
                        limit = SubElement(
                            joints[i],
