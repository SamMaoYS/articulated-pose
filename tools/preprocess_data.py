...
class PoseDataset:
        ...
        for ins in instances_per_obj:
            ...
            for art_index in os.listdir(base_path):
                ...
                try:
                    ...
                    meta_instance = yaml.load(meta_file, Loader=yaml.Loader)
    ...
    def __preprocess_and_save__(self, index):
        ...
        for link in range(num_parts):
            ...
            parts_world_orn[link] = np.array(pose_dict["obj"][link][5]).astype(
                np.float32
            )
        ...
        for s in range(num_parts):
            ...
            projected_points = projected_map[
                choose_x[s][:].astype(np.uint16), choose_y[s][:].astype(np.uint16), :
            ]
            projected_points = np.reshape(projected_points, [-1, 4])
            depth_channel = -projected_points[:, 3:4]
            cloud_cam = np.dot(
                projected_points[:, 0:2] - np.dot(depth_channel, projMat[0:2, 2:3].T),
            ...
            cloud_cam_real = np.dot(
                projected_points1[:, 0:2] - np.dot(depth_channel, projMat[0:2, 2:3].T),
                np.linalg.pinv(projMat[:2, :2].T),
            )
        ...
        for k in range(num_parts):
            ...
            part_points_space = np.concatenate(
                (parts_cloud_canon[k], np.ones((parts_cloud_canon[k].shape[0], 1))),
                axis=1,
            )
        ...
        for link in range(num_parts):
            ...
            parts_cloud_norm[link] = (
                (parts_cloud_urdf[link][:, :3] - base_p) * norm_factor
...
if __name__ == "__main__":
    ...
    split_dataset(
        ...
        [item],
        args,
        test_ins=infos.datasets[item].test_list,
        spec_ins=infos.datasets[item].spec_list,
        train_ins=infos.datasets[item].train_list,
    )
