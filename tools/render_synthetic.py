...
def render_data(
    ...
    if _RENDER_MODE == "random":
        ...
        steeringAngleArray = np.tile(
            np.linspace(0, 1, ARTIC_CNT).reshape(-1, 1), (1, num_joints)
        ) * (
            np.array([max_angles]).reshape(-1) - np.array([min_angles]).reshape(-1)
        ) + np.array(
            [min_angles]
        ).reshape(
            -1
    ...
    lightDirectionArray = 10 * np.random.rand(
        ARTIC_CNT, RENDER_NUM, 3
    )  # coming direction of light
    ...
    while simu_cnt < ARTIC_CNT:
        if (
            not os.path.exists(save_path + "/{}/{}/depth/".format(cur_urdf, simu_cnt))
        ) and _CREATE_FOLDER:
        ...
        print(
            "Rendering with joint angles {}".format(
        ...
        for m in range(10):
            ...
            q1 = lstate1[5]
            angle_diff = (
                2 * np.arccos(min(1, np.sum(np.array(q0) * np.array(q1)))) / np.pi * 180
            )
        ...
        for i in range(RENDER_NUM):
            ...
            while flag:
                ...
                if img_id < RENDER_NUM and _RENDER_FLAG:
                    camTargetPos = (
                        0.8 * min_dis * (np.random.rand(3) - 0.5) + center_pts[0]
                    )
                    nowTime = time.time()
                    ...
                    depth = (
                        255.0
                        * nearPlane
                        / (farPlane - (farPlane - nearPlane) * depth_raw)
                    )  # *farPlane/255.0
                    far = farPlane
                    near = nearPlane
                    depth_to_save = (
                        ...
                        * far
                        * near
                        / (far + near - (far - near) * (2 * depth_raw - 1.0))
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", default="train", help="mode decides saving folder:train/demo"
    )
    parser.add_argument(
        "--roll", default="30,40", help="camera view angle", required=False
    )
    parser.add_argument(
        "--pitch", default="30,40", help="camera view angle", required=True
    )
    parser.add_argument(
        "--yaw", default="30,40", help="camera view angle", required=True
    )
    parser.add_argument(
        "--min_angles", default="30,40,50", help="minimum joint angles", required=True
    )
    parser.add_argument(
        "--max_angles", default="30,40,50", help="maximum joint angles", required=True
    )
    parser.add_argument("--cnt", default=30, help="count of articulation change")
    parser.add_argument(
        "--num", default=10, help="number of rendering per articulation"
    )
    ...
    if is_debug:
        for instance in all_ins:
            render_data(
                ...
                args=args,
                _WRITE_FLAG=_WRITE,
                _RENDER_FLAG=_RENDER,
                _CREATE_FOLDER=_CREATE,
                RENDER_NUM=num_render,
                ARTIC_CNT=cnt_artic,
                _USE_GUI=_USE_GUI,
                _IS_DUBUG=is_debug,
            )
