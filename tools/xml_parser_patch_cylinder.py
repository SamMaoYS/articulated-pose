        ...
        if i == 0:
            ...
            if links_shape[i] == "cylinder":
                shape = SubElement(
                    geometry,
                    "cylinder",
                    length=str(links_h[i]),
                    radius=str(links_h[i] / 4),
                )
            elif links_shape[i] == "box":
                shape = SubElement(
                    geometry,
                    "box",
                    size="{} {} {}".format(links_w[0], links_w[1], links_h[i]),
                )
            ...
            # visual for joint
            origin_joint = SubElement(
                visual[1], "origin", rpy="0.0 1.5707 0", xyz="0 0 0"
            )
            geometry_joint = SubElement(visual[1], "geometry")
            shape_joint = SubElement(
                geometry_joint,
                "cylinder",
                length=str(links_w[0]),
                radius="{}".format(links_h[i] / 4),
            )
        ...
        if i == 0:
            if links_shape[i] == "box":
                origin = SubElement(
                    joints[i],
                    "origin",
                    xyz="0 {} {}".format(links_w[1] / 2, (links_h[i]) / 2),
                    rpy="0 0 0",
                )
                ...
                origin = SubElement(
                    joints[i],
                    "origin",
                    xyz="0 0 {}".format((links_h[i]) / 2 - 0.005),
                    rpy="0 0 0",
                )
        else:
            if links_shape[i] == "box":
                origin = SubElement(
                    joints[i],
                    "origin",
                    xyz="0 {} {}".format(links_w[1], links_h[i]),
                    rpy="0 0 0",
                )
            elif links_shape[i] == "cylinder":
                origin = SubElement(
...
if __name__ == "__main__":
    for i in range(2):
        ...
        print(
            "Generating {}th urdf file with {} parts>>>>>>>>>".format(i + 1, parts_num)
        )
