...
if __name__ == "__main__":
    domain = "unseen"
    filename = f"predictions_{domain}.json"
    ...
    for key, all_joints in predictions.items():
        ...
        for part_id, joint in all_joints.items():
            ...
            joints[key][part_id]["start_pt"] = joint_pt.tolist()
            joints[key][part_id]["end_pt"] = end_pt.tolist()
            joints[key][part_id]["axis"] = ((end_pt - joint_pt) / np.linalg.norm((end_pt - joint_pt))).tolist()
            joints[key][part_id]["error"] = float(angle)
            ...
            joints[key][part_id]["2d_start"] = [sx, sy]
            joints[key][part_id]["2d_end"] = [ex, ey]
    ...
    with open(f"prior_pred_{domain}.json", "w+") as f:
